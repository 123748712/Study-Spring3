...
public class PasswordEncoderTest {
	...
	public void myTest() throws Exception {
		String mySQL = "insert into mymem(userid, userpw) values(?, ?)";
		for (int i = 1; i <= 10; i++) {
			Connection conn = dataSource.getConnection();
			PreparedStatement psmt = conn.prepareStatement(mySQL);
			psmt.setNString(1, "user" + i);
			psmt.setNString(2, passwordEncoder.encode("user"+i));
			psmt.executeUpdate();
...
//	@Disabled // junit5 테스트실행이 되어도 실행되지 않게하는 어노테이션
	public void myTest2() throws Exception {
		String mySQL = "insert into myauth(userid, userauth) values(?, ?)";
		for (int i = 1; i <= 10; i++) {
			Connection conn = dataSource.getConnection();
			PreparedStatement psmt = conn.prepareStatement(mySQL);
			psmt.setNString(1, "user" + i);
			if(i <=5) {
				psmt.setNString(2, "ROLE_ADMIN"); // ROLE_권한
			} else {
				psmt.setNString(2, "ROLE_MEMBER");
			}
			psmt.executeUpdate();
